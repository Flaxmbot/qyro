...
class RedisQyroMemory:
    ...
    def __init__(
        ...
        Environment Variables:
            QYRO_REDIS_URL: Full Redis URL (takes precedence if set)
            QYRO_REDIS_HOST: Redis host (default: 'redis' for Docker)
            QYRO_REDIS_PORT: Redis port (default: 6379)
            QYRO_REDIS_PASSWORD: Redis password (optional)
        """
        import os

        # Check for QYRO_REDIS_URL first (takes precedence)
        redis_url = os.getenv('QYRO_REDIS_URL')
        if redis_url:
            ...
            host = host or os.getenv('QYRO_REDIS_HOST', 'redis')
            port_str = os.getenv('QYRO_REDIS_PORT', '6379')
            try:
                port = port if port is not None else int(port_str)
            except ValueError:
                port = 6379
            password = password or os.getenv('QYRO_REDIS_PASSWORD')
...
def create_redis_memory(
    ...
    Args:
        host: Redis server hostname (overrides env var if provided)
        port: Redis server port (overrides env var if provided)
        db: Redis database number
        password: Redis password (overrides env var if provided)
