...
class QyroConfig:
    ...
    def __post_init__(self):
        ...
        if self.redis_host == "localhost":
            self.redis_host = os.getenv("QYRO_REDIS_HOST", "localhost")
        if self.redis_port == 6379:
            self.redis_port = int(os.getenv("QYRO_REDIS_PORT", "6379"))
        if self.redis_password is None:
            self.redis_password = os.getenv("REDIS_PASSWORD")
        
        # Kafka configuration
        if self.kafka_bootstrap_servers == "kafka:29092":
            self.kafka_bootstrap_servers = os.getenv("QYRO_KAFKA_BOOTSTRAP_SERVERS", "kafka:29092")
