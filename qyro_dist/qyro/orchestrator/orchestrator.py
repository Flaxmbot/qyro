...
class QyroOrchestrator:
    ...
    def __init__(
        ...
        use_services: bool = True,  # If False, runs without Redis/Kafka
    ):
        self.qyro_file = qyro_file
        self.config = config
        self.use_services = use_services  # If False, skip Redis/Kafka entirely
    ...
    def start(self):
        ...
        try:
            ...
            # Initialize Kafka manager only if services are enabled
            if self.use_services:
                self._initialize_kafka_manager()
            else:
                logger.info("Running in standalone mode - Redis/Kafka disabled.")
                self.kafka_manager = None

            # Initialize Redis memory only if services are enabled
            if self.use_services:
                self._initialize_redis_memory()
            else:
                self._redis_available = False
                logger.info("Running without Redis - shared state disabled.")
