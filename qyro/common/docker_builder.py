...
class QyroDockerBuilder:
    ...
    def _generate_dockerfile(self, deps: Dict[str, Any]):
        ...
        dockerfile = [
            ...
            "# --- Qyro Library ---",
            # Copy from qyro_dist/qyro (copied by _copy_qyro_lib) to /app/qyro
            "COPY qyro_dist/qyro /app/qyro",
            "ENV PYTHONPATH=/app",
        ...
        py_deps.extend(["redis", "confluent-kafka", "rich", "pydantic", "docker"]) # Core deps

        with open(self.output_dir / "requirements.txt", 'w') as f:
            f.write("\n".join(py_deps))

        dockerfile.extend([
            "COPY qyro_dist/requirements.txt .",
            "RUN pip install -r requirements.txt"
        ])

        # --- Rust Deps ---
        # Generate a dummy Cargo.toml to cache dependencies?
        # For now, we'll compile at runtime or in a later step.
        # But to be 'functional', we need to compile.

        # Copy Source
        dockerfile.extend([
            "# --- Application Source ---",
            "COPY qyro_dist/modules /app/modules",
            "COPY . .", # Copy everything else just in case

            "# --- Entrypoint ---",
            "CMD [\"python\", \"-m\", \"qyro.cli.cli\", \"run-internal\"]"
        ])

        # Note: "run-internal" is a new CLI command we need to add to run inside Docker
...
services:
  qyro-app:
    build:
      context: .
      dockerfile: qyro_dist/Dockerfile
    volumes:
      - .:/app
    environment:
      - QYRO_REDIS_HOST=redis
      - QYRO_REDIS_PORT=6379
      - QYRO_KAFKA_BOOTSTRAP_SERVERS=kafka:9092
    depends_on:
      - redis
      - kafka
