  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[85vh] text-center relative px-4 py-16">
      ...
      <div className="animate-slide-up inline-flex items-center gap-3 px-4 py-2 border border-white/10 bg-black/60 backdrop-blur-md text-xs font-mono uppercase tracking-widest mb-10 shadow-[0_0_15px_rgba(255,255,255,0.05)] hover:border-white/40 transition-colors text-gray-300">
        <span className="size-1.5 bg-white shadow-[0_0_8px_white] rounded-full animate-pulse"></span>
      ...
      <p className="animate-slide-up text-sm text-gray-500 mb-12 max-w-lg leading-relaxed font-mono" style={{ animationDelay: "0.4s" }}>
        // UNIFIED EXECUTION LAYER <br />
        // ZERO-LATENCY BINDINGS DETECTED
      </p>
      ...
      <div className="flex flex-col sm:flex-row items-center gap-6 w-full sm:w-auto mb-20 animate-slide-up" style={{ animationDelay: "0.6s" }}>
        ...
        <a href="#" className="flex items-center justify-center gap-3 w-full sm:w-auto px-10 py-4 bg-transparent border border-white/20 text-gray-400 hover:text-white hover:border-white/60 transition-all font-mono text-xs uppercase tracking-widest group">
        ...
        <HoloPanel>
          <div className="flex items-center justify-between mb-6 border-b border-white/10 pb-4">
            <div className="text-[10px] text-gray-500 font-mono uppercase tracking-widest">
              <span className="text-white mr-2">&gt;&gt;&gt;</span>main.qyro
            </div>
          ...
          <pre className="font-mono text-xs sm:text-sm leading-loose text-left overflow-x-auto whitespace-pre-wrap">
            <span className="text-gray-500"># System Configuration</span>{'\n'}
            <span className="text-white">import</span> {'{ Runtime }'} <span className="text-white">from</span> <span className="text-gray-400">'@qyro/core'</span>;{'\n'}
            <span className="text-gray-500">// Initialize runtime sequence</span>{'\n'}
            <span className="text-white">const</span> runtime = <span className="text-white">new</span> <span className="text-blue-300">Runtime</span>{'{'}{'\n'}
            {'  '}<span className="text-gray-400">mode</span>: <span className="text-white">'SANDBOX_STRICT'</span>,{'\n'}
            {'  '}<span className="text-gray-400">modules</span>: [<span className="text-green-300">'python'</span>, <span className="text-orange-300">'rust'</span>]{'\n'}
            {'}'});{'\n'}
            <span className="text-white">await</span> runtime.<span className="text-blue-300">execute</span>(<span className="text-gray-400">`{'\n'}
              def process_signal(input):{'\n'}
                  return "Signal processed: " + input{'\n'}
            `</span>);
          </pre>
        </HoloPanel>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/10 mt-32 w-full max-w-5xl border border-white/10 rounded-sm overflow-hidden animate-slide-up" style={{ animationDelay: "1s" }}>
        {[
          { title: 'Zero Latency', icon: 'bolt', desc: 'Direct memory access between guest languages and host runtime via shared buffers.' },
          { title: 'Isolation', icon: 'lock_open', desc: 'Secure execution environment with granular permission controls for file system.' },
          { title: 'Global Edge', icon: 'public', desc: 'Deploy your polyglot logic to 35+ regions instantly with single command.' },
        ].map((feature, i) => (
      ...
      <footer className="w-full border-t border-white/10 py-12 px-8 bg-black z-10 relative mt-32">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6">
          <div className="flex items-center gap-2">
            ...
            <p className="text-gray-600 text-xs font-mono uppercase">All Systems Normal</p>
          </div>
          <div className="flex items-center gap-8">
            {['Privacy', 'Terms', 'Status'].map(item => (
              <a key={item} href="#" className="text-gray-600 hover:text-white transition-colors text-xs font-bold uppercase tracking-widest">
                {item}
              </a>
            ))}
          </div>
          <p className="text-gray-800 text-xs font-mono">© 2024 QYRO INC.</p>
        </div>
      </footer>
    </div>
  );
